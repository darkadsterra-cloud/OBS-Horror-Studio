...
// Updated overlays.ts with performance fixes and fit-to-page
// Add to existing overlays.ts

// Optimize particle calculations
export function optimizedTickParticles(
  particles: OverlayParticle[],
  W: number,
  H: number,
  params: OverlayParams,
  ...
  particles.forEach(p => {
    ...
    p.life = Math.max(0, p.life - 0.002 * timeScale);
    
    // Boundary check with margin
    const margin = 100;
    if (p.life <= 0 || p.y > H + margin || p.y < -margin || p.x > W + margin || p.x < -margin) {
      p.life = 1;
...
// Spatial hashing for collision/selection (future use)
export class SpatialGrid {
  private cells: Map<string, OverlayParticle[]> = new Map();
  ...
  insert(particle: OverlayParticle, x: number, y: number) {
  ...
  query(x: number, y: number, radius: number): OverlayParticle[] {
    const results: OverlayParticle[] = [];
...
export function batchDrawParticles(
  ctx: CanvasRenderingContext2D,
  particles: OverlayParticle[],
  drawFn: (p: OverlayParticle) => void
): void {
  ctx.save();
  particles.forEach(drawFn);
  ctx.restore();
}
