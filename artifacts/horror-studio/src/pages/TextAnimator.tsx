// TextAnimator.tsx — Fixed Version
// Bugs Fixed:
// 1. tickParticles never called → added to render loop
// 2. isTransitioning/transitionProgress as state in render dep array → converted to refs
// 3. bgTransform.opacity ignored in drawBg → now applied via ctx.globalAlpha
// 4. Dual fit-mode conflict (bgObjectFit vs bgTransform.fitMode) → unified to bgTransform.fitMode
// 5. Video object URL memory leak → URL.revokeObjectURL on cleanup
// 6. Dead imports removed (calculateMediaDrawParams, MEDIA_TRANSITIONS, TRANSITION_CATEGORIES, DEFAULT_PARAMS)
...
import {
  ...
  type OverlayDef, type OverlayParticle, type OverlayParams, type CustomOverlay,
} from "@/data/overlays";
...
function MakeLayer(p: Partial<TextLayer> = {}): TextLayer {
  return {
    ...
    text: "STARTING SOON", x: 0.5, y: 0.5,
    ...
    glowEnabled: false, glowColor: "#ff0000",
    letterSpacing: 0, animation: "none", _w: 0, _h: 0,
    ...p,
  };
}
...
function TemplatePreview({ template, text }: { template: TemplateData; text: string }) {
  ...
  return (
    <div className="h-full flex items-center justify-center overflow-hidden" style={{ background: bg }}>
      <span
        className="text-sm font-bold block text-center px-2 leading-tight"
        style={{
          fontFamily: `'${template.font}',sans-serif`,
          color: template.colors[0],
          textShadow: template.glow ? `0 0 10px ${template.colors[0]}` : "none",
        }}
      >
...
function TemplateCard({ template, selected, onClick, text }: {
  ...
  return (
    <div
      onClick={onClick}
      className={`rounded border cursor-pointer transition-all overflow-hidden ${
        selected
...
export default function TextAnimator() {
  ...
  // FIX #4: single source of truth for fit mode — bgTransform.fitMode only
  const [bgTransform, setBgTransform] = useState<MediaTransform>(DEFAULT_MEDIA_TRANSFORM);

  // Selected media for transform drag
  ...
  // FIX #2: transitions as refs so render loop doesn't restart every frame
  ...
  // Keep display state for button label only
  const [isTransitioningDisplay, setIsTransitioningDisplay] = useState(false);
  const textTransitionRef   = useRef(textTransition);
  const textTransParamsRef  = useRef(textTransitionParams);
  const mediaTransitionRef  = useRef(mediaTransition);
  const mediaTransParamsRef = useRef(mediaTransitionParams);

  useEffect(() => { textTransitionRef.current  = textTransition;     }, [textTransition]);
  useEffect(() => { textTransParamsRef.current  = textTransitionParams; }, [textTransitionParams]);
  useEffect(() => { mediaTransitionRef.current  = mediaTransition;    }, [mediaTransition]);
  useEffect(() => { mediaTransParamsRef.current = mediaTransitionParams; }, [mediaTransitionParams]);
  ...
  const selOvRef       = useRef(selectedOverlayInstance);
  ...
  useEffect(() => { selOvRef.current         = selectedOverlayInstance; }, [selectedOverlayInstance]);
  ...
  }), [canvasPreset, layers, activeOverlays, customOverlays, audioLibrary,
    textTransition, textTransitionParams, bgTransform, mediaTransition]);
  ...
  const allTemplates: TemplateData[] = [
    ...
    ...dbTemplates.map(t => ({
      ...
      animation: t.animation, colors: t.colors as string[],
      glow: t.glow, shadowEffect: t.shadowEffect,
      backgroundStyle: t.backgroundStyle, motionBehavior: t.motionBehavior,
      isPreset: t.isPreset,
    })),
  ];
  ...
  const selectTemplate = (tpl: TemplateData) => {
    ...
    if (selectedLayerId) {
      updateLayer(selectedLayerId, {
        color: tpl.colors[0], fontFamily: tpl.font, animation: tpl.animation,
        glowEnabled: tpl.glow, shadowEnabled: tpl.shadowEffect,
      });
    }
  };
  ...
  // FIX #5: revoke old object URL to prevent memory leak
  ...
  const clearBg = () => {
    if (bgVideo) bgVideo.pause();
    if (bgVideoUrlRef.current) { URL.revokeObjectURL(bgVideoUrlRef.current); bgVideoUrlRef.current = null; }
    setBgImage(null);
    setBgVideo(null);
    setBgTransform(DEFAULT_MEDIA_TRANSFORM);
  ...
  // FIX #2: triggerTransition uses refs, not state setters that cause re-renders
  const triggerTransition = useCallback((transitionId: string, params: TransitionParams = DEFAULT_TRANSITION) => {
    ...
    setIsTransitioningDisplay(true);
    ...
    const animate = () => {
      ...
      if (progress < 1) {
        ...
        setTimeout(() => {
          ...
          transitionProgressRef.current = 0;
          setIsTransitioningDisplay(false);
        }, 100);
      }
    };
    requestAnimationFrame(animate);
  ...
  const addOverlay = (defId: string) => {
    ...
    overlayParticlesRef.current[instanceId] = def.initParticles(
      presetRef.current.w, presetRef.current.h, def.params
    );
  ...
  const addCustomOverlay = (co: CustomOverlay) => {
    const instanceId = Math.random().toString(36).slice(2);
    const newOv: ActiveOverlay = {
      instanceId, defId: `custom:${co.id}`, label: co.name,
      emoji: co.isVideo ? "🎬" : "🖼️",
      params: {
    ...
    overlayParticlesRef.current[instanceId] = initCustomParticles(
      presetRef.current.w, presetRef.current.h, co
    );
    setActiveOverlays(prev => [...prev, newOv]);
    setEditingOverlay(instanceId);
    setSelectedOverlayInstance(instanceId);
  ...
  const updateOverlayParams = (instanceId: string, patch: Partial<OverlayParams>) => {
    setActiveOverlays(prev => prev.map(o => {
      ...
      if (patch.count !== undefined || patch.direction !== undefined ||
          patch.sizeMin !== undefined || patch.sizeMax !== undefined) {
  ...
  const finalizeCustomOverlay = () => {
    ...
    const co: CustomOverlay = {
      ...
      isGif: pendingIsGif, isVideo: pendingIsVideo,
  ...
  const handleAudioUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    const url = URL.createObjectURL(file);
    setAudioLibrary(prev => [...prev, {
      id: Math.random().toString(36).slice(2),
      name: file.name.replace(/\.[^/.]+$/, ""),
      type: "uploaded", url, duration: 0,
    }]);
  };
  ...
  const saveProject = () => {
    const state: ProjectState = {
      ...
      layers: layers.map(({ _w, _h, ...rest }) => rest),
      activeOverlays, customOverlays,
      bgColor: "#000000", audioLibrary,
      textTransition, textTransitionParams,
      bgTransform, mediaTransition,
    };
    const blob = new Blob([JSON.stringify(state, null, 2)], { type: "application/json" });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a");
    a.href = url; a.download = `horror-project-${Date.now()}.json`; a.click();
  };
  ...
  // ─── Render Loop ────────────────────────────────────────────────────────────
  // FIX #2: no transition state in dep array → loop never restarts mid-transition
  useEffect(() => {
    ...
    // FIX #3 & #4: apply bgTransform.opacity and use bgTransform.fitMode
    const drawBg = (W: number, H: number) => {
      const transform = bgTransformRef.current;
      const img       = bgImageRef.current;
      const vid       = bgVideoRef.current;
      const tpl       = templateRef.current;
      const fit       = transform.fitMode ?? "cover"; // FIX #4: unified fit mode
      ...
      if (media) {
        ...
        ctx.save();
        ctx.globalAlpha = transform.opacity ?? 1; // FIX #3: apply opacity
        ...
        if (fit === "contain") {
          ...
          dw = sw; dh = sh; dx = (W - dw) / 2; dy = (H - dh) / 2;
        }
    ...
    const render = (timestamp: number) => {
      ...
      // Background with optional transition
      ctx.save();
      const isT = isTransitioningRef.current;
      const tp  = transitionProgressRef.current;
      if (mediaTransitionRef.current !== "none" && isT) {
        applyTransition(ctx, W, H, tp, mediaTransitionRef.current, mediaTransParamsRef.current, true);
      }
      drawBg(W, H);
      if (mediaTransitionRef.current !== "none" && isT) cleanupTransition(ctx);
      ctx.restore();

      // FIX #1: tick particles BEFORE drawing so they actually move
      activeOverlaysRef.current.forEach(ov => {
        const ps = overlayParticlesRef.current[ov.instanceId];
        if (ps) tickParticles(ps, W, H, ov.params);
      });
      ...
      // Text layers with optional transition
      layersRef.current.forEach(l => {
        ctx.save();
        if (textTransitionRef.current !== "none" && isT) {
          applyTransition(ctx, W, H, tp, textTransitionRef.current, textTransParamsRef.current, true);
        }
        drawLayer(l, W, H, t);
        if (textTransitionRef.current !== "none" && isT) cleanupTransition(ctx);
        ctx.restore();
      });
  ...
  }, [canvasPreset]); // FIX #2: minimal dep array — transitions use refs, no restarts
  ...
  const handleCanvasMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // Check overlay hit (reverse = top first)
    ...
    // Text layer rotate/resize handle check
    if (selectedLayerId) {
      ...
      if (sel) {
        ...
        const cos = Math.cos(sel.rotation), sin = Math.sin(sel.rotation);
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (ovDragging.current && selectedMedia) {
        ...
        updateLayer(dragId.current, { fontSize: Math.round(Math.max(8, Math.min(600, resizeStartSize.current + dy * 0.5))) });
    ...
    const onUp = () => {
      dragging.current  = false; resizing.current  = false; rotating.current  = false;
      dragId.current    = null;
  ...
  }, [canvasPreset, updateLayer, selectedOverlayInstance, selectedMedia]);
  ...
  const stopRecording = () => {
    ...
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== "inactive") {
      ...
      setTimeout(() => { if (mediaRecorderRef.current?.state !== "inactive") mediaRecorderRef.current?.stop(); }, 200);
    }
    setRecording(false);
  };
  ...
  const filteredOverlays = [
    ...
    ...(Array.isArray(customOverlays) ? customOverlays : [])
    ...
    <div className="h-full flex overflow-hidden">
      ...
      <aside className="w-64 flex-shrink-0 border-r border-red-900/20 bg-[#050508] flex flex-col overflow-hidden">
        <div className="p-3 border-b border-red-900/20">
          ...
          />
          <button
            onClick={() => { const l = MakeLayer({ text: newText || "TEXT", fontFamily: FONT_OPTIONS[0], color: selectedTemplate.colors[0] }); setLayers(p => [...p, l]); setSelectedLayerId(l.id); }}
        ...
        <div className="p-2 border-b border-red-900/20">
          ...
          <div className="space-y-1 max-h-28 overflow-y-auto">
            {layers.map(l => (
              <div
                key={l.id}
                onClick={() => { setSelectedLayerId(l.id); setSelectedOverlayInstance(null); setSelectedMedia(null); }}
              ...
              >
                ...
                <button
                  onClick={e => { e.stopPropagation(); setLayers(p => p.filter(x => x.id !== l.id)); if (selectedLayerId === l.id) setSelectedLayerId(null); }}
                  className="text-zinc-600 hover:text-red-400 text-sm px-0.5"
                >×</button>
              </div>
            ))}
          </div>
          ...
          <div className="p-2 border-b border-red-900/20">
            ...
            <div className="space-y-1 max-h-24 overflow-y-auto">
              {activeOverlays.map(ov => (
                <div
                  key={ov.instanceId}
                  onClick={() => { setSelectedOverlayInstance(ov.instanceId); setSelectedLayerId(null); setSelectedMedia(null); setEditingOverlay(ov.instanceId); }}
                  className={`flex items-center gap-1.5 px-2 py-1 rounded cursor-pointer text-xs border transition-all ${ov.instanceId === selectedOverlayInstance ? "bg-purple-900/30 border-purple-700/40 text-purple-200" : "bg-zinc-800/40 border-zinc-800/30 text-zinc-400 hover:border-zinc-600/40"}`}
                >
      ...
      <div className="flex-1 flex flex-col p-3 overflow-hidden min-w-0">
        ...
        <div className="relative mb-2">
          <button onClick={() => setShowSizeMenu(v => !v)} className="flex items-center gap-2 px-3 py-1.5 rounded bg-zinc-800/60 border border-zinc-700/40 text-xs text-zinc-300 hover:border-purple-700/40 transition-colors w-full max-w-sm">
            ...
            <span className="text-zinc-500 text-[10px] font-bold">{canvasPreset.w}×{canvasPreset.h}</span>
          </button>
          {showSizeMenu && (
            <div className="absolute z-20 mt-1 w-full max-w-sm rounded bg-zinc-900 border border-zinc-700/40 shadow-xl">
              {CANVAS_PRESETS.map(p => (
                <button key={p.label} onClick={() => { setCanvasPreset(p); setShowSizeMenu(false); }}
                  className={`block w-full text-left px-3 py-1.5 text-xs hover:bg-zinc-800/60 ${canvasPreset.label === p.label ? "text-purple-300 bg-purple-900/10" : "text-zinc-400"}`}
                >{p.label}</button>
              ))}
            </div>
          )}
        ...
        <div className="mt-2 flex items-center gap-2 flex-wrap flex-shrink-0">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded border text-xs font-bold transition-all ${recording ? "bg-red-900/40 border-red-700/40 text-red-300 animate-pulse" : "bg-zinc-800/60 border-zinc-700/40 text-zinc-300 hover:border-red-700/40"}`}
          >{recording ? `⏹ Stop (${fmt(recordingTime)})` : "🔴 Record"}</button>
          ...
          <div className="flex items-center gap-2 text-[10px] text-zinc-500">
            {isSaving
              ...
              : lastSaved
              ? (<><span className="text-green-500">✓</span><span>Auto-saved {lastSaved.toLocaleTimeString()}</span></>)
          ...
          <button onClick={() => exportProjectBackup(getProjectData())} className="px-3 py-1.5 rounded bg-yellow-900/40 border border-yellow-700/40 text-xs text-yellow-300 hover:bg-yellow-900/60">📥 Export Backup</button>
        </div>

        {showRecordings && (
          <div className="mt-2 rounded border border-zinc-800/40 bg-[#0a0a14] p-2 max-h-32 overflow-y-auto">
            {recordings.length === 0
              ? <div className="text-xs text-zinc-600">No recordings yet</div>
              : recordings.map((r, i) => (
      ...
      {/* Right Sidebar */}
      <aside className="w-72 flex-shrink-0 border-l border-red-900/20 bg-[#050508] flex flex-col overflow-hidden overflow-y-auto">
        ...
        {(bgImage || bgVideo) && (
          <div className="p-3 border-b border-blue-900/20">
            ...
            <div className="space-y-2">
              <div>
                {/* FIX #4: now controls bgTransform.fitMode which is what drawBg reads */}
              ...
              <div>
                {/* FIX #3: opacity slider now actually does something */}
        ...
        <div className="p-3 border-b border-purple-900/20">
          ...
          <div className="space-y-2">
            ...
            <button
              ...
              disabled={textTransition === "none" || isTransitioningDisplay}
            ...
            >
              {isTransitioningDisplay ? "⏳ Playing..." : "▶ Play Transition"}
            </button>
          </div>
        </div>
            ...
            <div className="space-y-2">
              ...
              {showAddCustom && (
                <div className="space-y-2 p-2 rounded bg-zinc-900/50 border border-zinc-800/30">
                  ...
                  {pendingCustomFile && (
                    <div className="text-xs text-zinc-400">{pendingIsVideo ? "🎬 Video" : pendingIsGif ? "🎞️ GIF" : "🖼️ Image"} selected</div>
                  )}
                  <div className="flex gap-1">
